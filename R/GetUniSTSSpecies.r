GetUniSTSSpecies <-
function(stsid,showurl=FALSE)
   {
   if(length(stsid)>1)
     stop("Please provide only one id number at a time for this function")
   for(i in 1:nchar(stsid))
     {
     if(substr(stsid,i,i) %in% LETTERS | substr(stsid,i,i) %in% letters)
        stop("Please specify an ID number not a search term. Use GetIDs or GetUniSTSFromName")
     }
