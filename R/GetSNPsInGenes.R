...
function(locusIDs,batchsize=200,MaxRet=30000,showurl=FALSE,quiet=TRUE,smt=FALSE,sme=FALSE)
   ...
   getURL<-paste(URLdef$front,"esearch.fcgi?db=snp&term=",locusIDs,"[LOCUS_ID]+AND&retmax=",formatC(MaxRet,digits=9,width=1),"&rettype=FASTA",URLdef$back,sep="")
   listofSNPs<-rep("",MaxRet)
   webget<-get.file(getURL,showurl=showurl,quiet=quiet,clean=FALSE)
   listofSNPs<-getListFromXML(webget,MaxRet=MaxRet,sme=sme,smt=smt)
