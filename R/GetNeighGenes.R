...
function(chr,chrpos,FlankingDistance=100000,showurl=FALSE,hyper="HYPERLINK",full=FALSE,web=TRUE,html=FALSE,org="human",cg=TRUE,div="**",sme=FALSE,smt=FALSE,pbar=TRUE)
   ...
   Frame1$LowPoint<-as.numeric(chrpos)-FlankingDistance
   Frame1$HighPoint<-as.numeric(chrpos)+FlankingDistance
   ...
   for(i in 1:length(chrpos)) 
      {   
       ...
       locusID<-GetRegion("gene",Frame1$chr[i],Frame1$LowPoint[i],Frame1$HighPoint[i],cg=cg,org=org,sme=sme,smt=smt) 
