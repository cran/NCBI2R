process.gene.table.int.old<-function(webget)
   {
   CheckThing<-FALSE
   linenumber<-grep("mRNA   bp   exons   Protein   aa   exons",webget)
   if(length(linenumber)!=0)
      {
      if(length(unlist(strsplit(webget[linenumber+1],"  ")))==6)
         {
         CheckThing<-TRUE
         CurrentRow<-1
         dummy<-0
         ...
         CheckThing<-CurrentRow<length(webget)
         }
      }
   if(CheckThing)
      {
      ...
      for(i in 1:TotalExonSets)
         ...
         while(substr(webget[CurrentRow+1],nchar(webget[CurrentRow+1])-1,nchar(webget[CurrentRow+1]))=="bp" & CurrentRow<length(webget))
            ...
            while(currentposition<nchar(TL))
               ...
               ExonInfo$Start[WriteHere]<-as.numeric(substr(TL,startposition,currentposition-2))
               startposition<-currentposition
               while(substr(TL,currentposition,currentposition+2)!= "   ")
                  currentposition<-currentposition+1
               ExonInfo$Stop[WriteHere]<-as.numeric(substr(TL,startposition+2,currentposition-1))
               startposition<-currentposition
               while(substr(TL,currentposition,currentposition+2)!= " bp")
                  currentposition<-currentposition+1
               ExonInfo$Size[WriteHere]<-as.numeric(substr(TL,startposition+3,currentposition-1))  
